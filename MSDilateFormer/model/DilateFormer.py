...
class Dilateformer_MSDA(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x)   #  torch.Size([32, 96, 10, 256])
        for stage in self.stages:   #  torch.Size([32, 192, 5, 128])
            x = stage(x)
        x = x.flatten(2).transpose(1, 2).reshape(batch_size, -1, self.embed_dim)   #  torch.Size([32, 1280, 96])
        x = self.norm(x)  # B L C     #  torch.Size([32, 1280, 96])
...
class Dilateformer_MHSA(nn.Module):
    def __init__(self, H, W, Ph, Pw, in_chans, embed_dim, hidden_dim,
                 depths, num_heads, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.1,
                 norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 merging_way='conv3_2',
                 patch_way='nonoverlaping',
                 cpe_per_satge=False, cpe_per_block=True):
...
if __name__ == "__main__":
    x = torch.rand([32, 32, 20, 1024])
    m = Dilateformer_MHSA(H=20, W=1024, Ph=2, Pw=4, in_chans=32, 
